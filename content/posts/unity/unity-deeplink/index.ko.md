...
summary: "로컬 빌드에서는 열리던 유니버설 링크가 배포본에서 실패한 원인을 최종 앱 서명과 재서명 로그로 추적했습니다."
tags: ["111퍼센트", "유니티", "딥링크", "iOS", "젠킨스"]
categories: ["유니티"]
menu:
  sidebar:
...
### 1. OneLink와 AASA가 유효한지 확인 {#verify-onelink-aasa}
...
### 2. Unity PostBuildProcess에서 entitlement 연결 확인 {#verify-project-entitlements}
...
### 3. 최종 IPA의 코드사인 정보를 직접 검증 {#verify-final-signature}
...
#### 문제가 있는 Jenkins IPA의 entitlement 출력
...
### 4. Jenkins 빌드 파이프라인의 경계 좁히기 {#isolate-build-stage}
...
### 5. resign 로그에서 원인 확정 {#verify-resign-log}
